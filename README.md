...
Every signed-in account has a deliberately small self-service page at `https://bonifacio.work/sso/user/`. It shows only that account's own identity, permitted-service links, and password-change form after current-password verification; it never exposes the subscriber list, account issuance, role or application-grant controls, activation controls, or another user's data. The landing-page utility link is therefore labelled `내 정보`, not SSO Admin.

Central account management is a separate surface at `https://bonifacio.work/sso/admin/`, available only to `admin` and `chief-admin`. The canonical role hierarchy is `user < admin < chief-admin`; `developer` has been removed. A `chief-admin` reaches every protected application, while `user` and `admin` accounts reach only the applications explicitly checked in the administration console. That console supports per-user app grants, account issuance, one-time temporary password resets, role changes, and activation controls, but deliberately has no signed-in-user password panel: administrators change their own passwords through `/sso/user/` like everyone else. Only a `chief-admin` may manage privileged accounts, while an `admin` may manage ordinary users. Public self-registration and email-based recovery remain closed while outbound email and identity recovery are unavailable. The account service is the sole writer of the file-backed directory, avoiding lost updates between independent processes.

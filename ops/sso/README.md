...
Use `https://bonifacio.work/sso/admin/` after signing in as an enabled member of `owners`. New users receive a 24-character temporary password which is returned once and never written to the audit log or user listing. Send it through a private channel. Owners also issue later password resets from this screen. Email-based password recovery, anonymous registration, and Authelia's independent file-rewriting password-change flow remain disabled until a single serialized recovery workflow is available.

Central usernames and email addresses are immutable. Downstream applications establish a one-time link to the immutable username (`Remote-User`); editing the email later would make collision recovery ambiguous. Disable a mistaken account and issue a corrected one instead. The UI prevents the active administrator from removing their own rights and prevents disabling the last enabled owner.

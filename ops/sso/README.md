...
Only a `chief-admin` can change another account's password, including an ordinary user's password, by entering and confirming the desired value in the password-change dialog. Ordinary administrators cannot use this function. `POST /sso/admin/api/users/:username/reset-password` enforces this restriction before validation and again inside the writer lock; it requires exactly `newPassword` and `confirmPassword`, hashes the chosen value, and returns only the new revision. It also rejects resetting the acting chief's own password: every role uses `/sso/user/` with current-password verification for self-service. Empty requests no longer generate a password. New-account issuance still uses a one-time generated credential.

The administrator and private self-service password forms accept 12–128 characters and require at least one lowercase letter, number, and special character, matching the configured Authelia standard policy. Uppercase letters are optional. Existing operator-provided bootstrap credentials may still be entered as the current password so they can be replaced. Every account should choose a long unique password. Per account, at most five actual current-password checks are accepted in a rolling 10-minute window, and at most three successful self-service changes are accepted in a rolling 24-hour window; malformed, CSRF-rejected, or stale-revision requests do not consume those quotas.
